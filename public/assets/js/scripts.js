...
function produceSen(senId) {
  ...
  $.ajax(senList).done(function(response) {
    ...
    console.log(senObject.results[0]);
    ...
    $("#cardlocation").append('<div class="card"><img class="img-fluid img-responsive" src="assets/images/senpics/' +
      ...
      senObject.results[0].first_name + ' ' + senObject.results[0].last_name + '</br>(' + senObject.results[0].current_party + '-' + senObject.results[0].roles[0].state + ')</h4></div></div>');
...
var accHandler = {
  ...
  senFollow: function() {
    ...
    var arrValue = $.inArray(senId, accHandler.userArr);
    if (arrValue === -1) {
      accHandler.userDatabase.ref(accHandler.uid).push(senId);
    } else {
      $('#senmodal').modal('hide');
      $("#success-modal").modal('show');
      $("#success-span").text("You are already following this senator.");
      $("#success-modal").modal('show');
    }
  },
